...
class Message extends DataObject {
	 ...
	 *
	 * N.B.  Functionality added to detect if the user has allowed for emails to be sent to them by
	 * the system - Adam Lewis 2011-11-30
	 *
	 *
	 */
				...
				if($author = $this->Author()) {
					...
					foreach($members as $member) {
						...
						if(!$thread->isDeletedForUser($member->ID)) {
							// Work out if the user allows emails to be sent to them
							if ($member->BuddyEmailToggle) {
